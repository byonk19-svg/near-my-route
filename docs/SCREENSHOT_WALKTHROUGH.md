...
Do not paste real schedules, patient names, clinical details, DOBs, MRNs, referring MDs, real phone numbers, real emails, private/home-health addresses, or facility-contact data into this workflow. The checked-in script supplies only synthetic/demo data and an isolated test-only non-placeholder contact value so the ready-to-text state can be captured. Generated screenshots are ignored by git.
