...
async function runWalkthrough(
  ...
  try {
    ...
    await page.getByRole("heading", { name: /near my route/i }).first().waitFor();
    ...
    await page.getByRole("heading", { name: "Import tomorrow's route" }).waitFor();
    ...
    const blockedMapsButton = await optionalFirstVisible(page.getByRole("button", { name: "Confirm locations for Maps" }));
    if (blockedMapsButton) {
      assert.equal(await blockedMapsButton.isDisabled(), true);
    } else {
      await page.getByText("No candidates matching filters").first().waitFor();
    }
    ...
    const blockedTextButton = await firstVisible(textFirst.getByRole("button", { name: "Enter real phone first" }), "blocked Text First action");
    assert.equal(await blockedTextButton.isDisabled(), true);
    assert.equal(await visibleCount(page.getByRole("button", { name: "Mark texted" })), 0);
    await capture(page, outputDir, manifestPrefix, "10-placeholder-phone-protection.png", "Outreach", `${deviceLabel} Text First blocked action before a real phone is entered.`, [
      "The app does not open Messages or log Texted today while the SLP Contact phone is a placeholder.",
      "Contact setup is available in-place for replacing the placeholder value.",
    ]);
    ...
    await clickVisible(page, "Copy message");
    await firstVisible(
      page.getByText(/Template copied/),
      "template copied feedback",
    );
    ...
    await firstVisible(page.getByText(/Added tentatively|Added to tentative route|Stop added to route successfully/), "tentative add-on confirmation");
...
async function clickImportTab(page: Page) {
  await clickVisible(page, /^(Import Schedule|Import)$/);
}

async function clickOutreachTab(page: Page) {
  await clickVisible(page, /^(Outreach|Outreach Logs)$/);
}
...
async function optionalFirstVisible(locator: Locator) {
  const count = await locator.count();
  for (let index = 0; index < count; index += 1) {
    const item = locator.nth(index);
    if (await item.isVisible()) return item;
...
> {
  ...
  if (await isReachable(baseUrl)) return { started: false };
